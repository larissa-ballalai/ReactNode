...
export default class ListeningB extends Component {
constructor(props) {
  super(props);
  ...
  if (this.state.edit !== undefined) {
    axios.get(apiURL() + "questions/" + this.props.match.params.id)
      .then((response) => {
        ...
        var audio = {
          blob: adminConfig().root + "lb/" + response.data.sentence,
        ...
        this.setState({
          ...
          audio: audio,
          file: { 
            blob: adminConfig().root + "back/lb/" + response.data.background, 
...
onSubmit = (e) => {
  e.preventDefault();
    ...
    uploadBackground(this, "back/lb")   

  ReactS3.uploadFile(this.state.audio.file, adminConfig('lb').config)
    .then((data) => {
      console.log(data);
    })
    .catch((err) => {
...
hideModal = () => {
  ...
  render() {
    this.state.edit = this.props.match.params.id;
      return (
          <BaseQuestion name="Listening B" color="card badge-pink">
            <form onSubmit={this.onSubmit}>
                ...
                <Multiplechoice sentence={this.state.sentence} values={this.state.values} choices={this.state.choices}
                ...
                <ImgBackground classname="btn btn-outline-pink " src="../../img/upfilepink.png " file={this.state.file} addFiles={this.addFiles} deleteFile={this.deleteFile} 
