...
addAudio = (e) => {
    ...
    var audio = {file: e.target.files[0], name: e.target.files[0].name, blob: blob}
        ...
        axios.get(apiURL() + 'questions/' + this.props.match.params.id)
            .then(response => {   
                ...
                for(var i=0; i < files.optionlist.length; i++ ) {
                    var img = adminConfig().root + "la/" + files.optionlist[i]
                    newfiles.push({ blob: img, 
                                    name: files.optionlist[i],
                                    file: null})}

                var audio = {blob: adminConfig().root + "la/" + response.data.sentence,
...
onSubmit = (e) => {
    ...
    ReactS3.uploadFile(this.state.audio.file, adminConfig('la').config)
        .then( (data) => { console.log(data) })
        .catch( (err) => { console.log(err) })  

    let optionlist = []
    for (let i = 0; i < this.state.files.length; i++) {
        ReactS3.uploadFile(this.state.files[i].file, adminConfig('la').config)
            .then( (data) => { console.log(data) })
            .catch( (err) => { console.log(err) })  
